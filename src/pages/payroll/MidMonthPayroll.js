...
const columns = [
  ...
  },
  {
    ...
    key: "action",
    render: (_, payroll) => (
      <SalarySlipPDF employee={payroll}/>
    ),
  },
];
...
const SalarySlipPDF = ({ employee }) => {
  ...
  const generatePDF = () => {
    ...
    const basicSalary = employee?.midMonthSalary;
    const loans = employee?.midMonthLoanDeduction;
    const nssf = employee?.nssfAmount / 2;

    // Calculate total earnings and deductions
    const totalEarnings = basicSalary;
    const totalDeductions = loans + nssf;

    // Add content to PDF
    const title = "MID MONTH SALARY SLIP";
    const titleWidth =
    ...
    pdf.setFontSize(12);
    pdf.text(`${employee?.firstName} ${employee?.middleName} ${employee?.lastName}`, rightAlignX, 30, { align: "right", fontSize: 10 });
    pdf.text(`${employee?.designation}`, rightAlignX, 36, { align: "right" });
    pdf.text(`${employee?.month} ${employee?.year}`, rightAlignX, 42, { align: "right" });
    ...
    pdf.autoTable({
      ...
      body: [
        ["Basic Salary", `${formatter.format(basicSalary)}`, "Loans", `${formatter.format(loans)}`],
        ...
        [
          ...
          "Total Deductions",
          { content: `${formatter.format(totalDeductions)}`, fontStyle: "bold" },
        ],
      ],
    ...
    const netSalaryString = `Net Salary: TZS ${formatter.format(employee?.midMonthNetSalary)}`;
    pdf.text(netSalaryString, rightAlignX, pdf.autoTable.previous.finalY + 10, {
      align: "right",
    });

    // Save the PDF
    pdf.save(`${employee?.firstName} ${employee?.middleName} ${employee?.lastName} Mid Month Salary Slip ${employee?.month} ${employee?.year}`);
  };
